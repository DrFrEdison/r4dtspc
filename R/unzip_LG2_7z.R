unzip.LG2.7z <- function(date = NA
  ...
  # Nur LG2 und SG Loeningen
  customerlist <- customerlist[ customerlist$LG != 3,]
  customerlist <- customerlist[ customerlist$LG != "SG3",]
  customerlist <- customerlist[ customerlist$line != "LG_LOE",]

  # Nur eine Linie?
  if( !is.na(line)){
    oo <- which( customerlist$line == line)
  } else {oo <- 1:nrow(customerlist)  }

  for(o in oo){

    # Folder paths ####
    zip.dir <- paste0(service_backup_path(as.character(customerlist[o,]$customer), as.character(customerlist[o,]$location), as.character(customerlist[o,]$line), dir_wd = dir_wd),"ZIP/")
    csv.dir <- paste0(service_backup_path(as.character(customerlist[o,]$customer), as.character(customerlist[o,]$location), as.character(customerlist[o,]$line), dir_wd = dir_wd),"CSV/")
    ref.dir <- paste0(service_backup_path(as.character(customerlist[o,]$customer), as.character(customerlist[o,]$location), as.character(customerlist[o,]$line), dir_wd = dir_wd), "ref/")
    drk.dir <- paste0(service_backup_path(as.character(customerlist[o,]$customer), as.character(customerlist[o,]$location), as.character(customerlist[o,]$line), dir_wd = dir_wd), "drk/")
    ...
    if(identical(zip.files, character(0))) next
    if(just.list==T) next
      ...
      if(length(ref$dat) > 0){
        ...
        fwrite(ref$final
               , paste0(ref.dir
                        ...
                        , as.character( unlist( customerlist[o,"customer"])), "_"
                        , as.character( unlist( customerlist[o,"location"])), "_"
                        , as.character( unlist( customerlist[o,"line"])), "_ref.csv")
               , sep = ";", dec = ",")
      }
      # drk ####
      setwd(paste(unzip.dir.copy,gsub(".zip","",unlist(zip.files)[j]),sep="/"))
      ...
      if(length(drk$dat) > 0){
        ...
        fwrite(drk$final
               , paste0(drk.dir
                        ...
                        , as.character( unlist( customerlist[o,"customer"])), "_"
                        , as.character( unlist( customerlist[o,"location"])), "_"
                        , as.character( unlist( customerlist[o,"line"])), "_drk.csv")
               , sep = ";", dec = ",")
      }
      # spc ####
      setwd(paste(unzip.dir.copy,gsub(".zip","",unlist(zip.files)[j]),sep="/"))
